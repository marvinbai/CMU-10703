...
def generate_expert_training_data(expert, env, num_episodes=100, render=True):
    ...
    """
    states=np.zeros((1,4))
    hot_actions=np.zeros((1,2))

    max_episode_int=2000000
    total_int_cnt=0
    end_flag=False
    episode_cnt=0
    ...
    while end_flag==False:
        ...
        while episode_flag==False: 
            ...
            q_values=expert.predict(state)
            action=one_hot(q_values)

            hot_action=np.zeros((1,2))
            hot_action[0,action]=1
            hot_actions=np.vstack((hot_actions,hot_action))

            state,reward, done,info=env.step(action)
            total_int_cnt=total_int_cnt+1
            episode_int_cnt=episode_int_cnt+1
            state=process_state(state)
            '''
            if done==True:
                print('Dead!!!!!!!!!!!!!!!!!!!!!')
            '''
            if (episode_int_cnt>=max_episode_int)|(done==True):
                episode_flag=True
        episode_cnt=episode_cnt+1
        print(episode_cnt)
        if episode_cnt>=num_episodes:
            end_flag=True
        length=states.shape[0]
            
    return states[1:length,:], hot_actions[1:length,:]
...
def one_hot(q_values):
    return np.argmax(q_values)
def run_learner_query_expert(env,learner,expert):
    output=0
    return output
...
class LossHistory(keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        self.accues.append(logs.get('acc'))
