...
print('Shape of infos is:', infos.shape)
print('Shape of next_states:', next_states.shape)
# print('Rewards:', rewards)

states_new = np.zeros(states.shape)
actions_new = np.zeros(actions.shape)
rewards_new = np.zeros(rewards.shape)
dones_new = np.zeros(dones.shape)
infos_new = np.zeros(infos.shape)
next_states_new = np.zeros(next_states.shape)


print('========== With Data Selection ==========')
states, actions, rewards, dones, infos, next_states = mujoco_mem_store.sample_dataSelection(BATCH_SIZE, state_size, 
    action_size, REPETITION_NUM)
print('Shape of states is:', states.shape)
print('Shape of actions is:', actions.shape)
print('Shape of rewards is:', rewards.shape)
print('Shape of dones is:', dones.shape)
...
print('rewards_index:', rewards_index)
...
# gradients = critic.action_gradients(states, actions)
...
actor.online_nn.save_weights('half_online_actor_0.h5')
...
while train_flag==False:
    ...
    if epi_cnt%5 == 0:
        ...
        actor.online_nn.save_weights('half_online_actor_'+str(epi_cnt)+'.h5')
